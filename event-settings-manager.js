    ...
    async updateEvent(userId) {
        ...
        if (savedInventory) {
            try {
                ...
                // ✅ שמור את המלאי תמיד - צריך אותו גם במחשב אחר!
                if (inventorySizeKB > 5000) {
                    console.warn('⚠️ מלאי גדול מאוד (>5MB) - השמירה עלולה לקחת זמן');
                }
            } catch (e) {
                console.warn('⚠️ לא ניתן לקרוא מלאי');
            }
        ...
        try {
            await this.withTimeout(
                ...
                30000, // 30 שניות - מספיק גם למלאי גדול בחיבור איטי
                'כתיבת עדכון אירוע'
            );
